...
setenv ovsapply 'fdt addr ${fdt_addr_r}; fdt resize; setexpr fdtovaddr ${fdt_addr_r} + 10000; for overlay in ${overlays}; do echo Applying DT overlay ${overlay}; load ${devtype} ${devnum}:${distro_bootpart} ${fdtovaddr} /boot/overlays/${overlay}.dtb && fdt apply ${fdtovaddr}; done'
