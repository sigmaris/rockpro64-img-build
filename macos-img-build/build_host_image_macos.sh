...
then
	...
	rm -rf tftpserver netboot.tar.gz debian.qcow authorized_keys installer.log
fi
...
curl --location --output netboot.tar.gz https://debian.osuosl.org/debian/dists/buster/main/installer-amd64/current/images/netboot/netboot.tar.gz
mkdir -p tftpserver
pushd tftpserver
